...
- **Opportunities / synthesis** — not in the original roadmap above at all; added because a real user asked for it. A dashboard tab (and `brainny propose` / `skills/brainny/synthesize.md`) where an assistant proposes that several already-captured ideas genuinely combine into something bigger — a tool, website, statistical module, business idea — with an honest 0.0–1.0 confidence weight. Principle #1 in its purest form: nothing deterministic proposes this, it's a pure semantic call, same GATE discipline (default NO) as every capture skill. `brainny/opportunities.py`, real and tested.
