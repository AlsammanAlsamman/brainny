...
23. Opportunities / synthesis — AI-proposed combinations, and a public example dashboard. ✅
    - User's ask, two parts: (a) a dashboard tab where an assistant
      assigns a weight to sets of captured ideas that could combine into
      a tool, website, statistical module, or business idea; (b) a
      separate, fabricated example dashboard for the README so visitors
      can see brainny's capabilities without looking at the maintainer's
      real central brain.
    - New schema types `OpportunityInput`/`Opportunity`/`Opportunities`
      (`brainny/schema.py`) — `weight: float` bounded `[0.0, 1.0]`, same
      convention as `Edge.confidence`; `idea_ids` requires at least 2
      (an opportunity is by definition a combination). New
      `brainny/opportunities.py` mirrors `graph.py`'s
      load/save/next_id shape exactly, living in a sibling
      `opportunities.json` next to `graph.json`.
    - New CLI command `brainny propose <entries.json> --project <p>
      --session <s>`: validates every `idea_ids` entry actually exists in
      that project's `graph.json` first — an opportunity referencing a
      dangling id is silently skipped with a clear stderr message, not
      accepted. Auto-mirrors to central the same way capture/attach do
      (`_sync_to_central` extended to also copy `opportunities.json`).
    - `viz.py` gained a third dashboard tab, "Opportunities": each
      proposal shown as a card (kind badge, weight as a percentage bar,
      summary, optional rationale, "built from" chips linking back to the
      Graph tab's matching idea). `save_html()` now transparently loads
      `opportunities.json` from whatever `out_dir` it's given — every
      existing call site (`capture`/`attach`/`sync`/`reassign`/`central`/
      `query`) picked this up with zero changes needed at any of them.
      `central.py` gained `build_merged_opportunities()`, parallel to
      `build_merged_graph()`, which — unlike the graph merge — DOES
      rewrite `idea_ids` to `"<project>::<id>"` so an opportunity's
      "built from" links still resolve correctly in the merged central
      view, where ids are namespaced for DOM/d3 identity (see step 19's
      `itemKey()`). The project filter (step 19) applies to opportunities
      too, derived from the same `"<project>::<id>"` prefix.
    - New skill `skills/brainny/synthesize.md` / `/brainny-synthesize`
      (project-local + global mirror + wired into `CLAUDE.md`): reviews a
      project's real captured ideas (not just the current session) for
      genuine combinations, same GATE discipline as every capture skill
      (default NO — most passes propose nothing), explicit honesty
      requirement on `weight` (a genuine confidence call, never inflated
      to look better), always responds with what it proposed or why
      nothing cleared the bar.
    - New `examples/demo/` — a committed, fabricated dashboard (16 ideas
      across 7 domains, every kind and origin, code/table/plot
      attachments, inline snippets, and 3 example Opportunities at
      varying weights), built with the real CLI (`brainny capture` /
      `attach` / `propose`) against a scratch directory, then copied in —
      never generated by hand-editing JSON, so it exercises the same
      code path real data does. README links to it live via
      `htmlpreview.github.io` (GitHub doesn't render `.html` files
      directly). Caught and cleaned up a real near-miss while building
      it: the demo capture auto-mirrored to the maintainer's actual
      central folder (step 17's auto-mirror doesn't know a project is
      "just a demo") — deleted the stray `brainny-demo` central copy
      immediately after finishing, confirmed the real central folder was
      back to exactly its 2 real projects.
    - 20 new tests across `test_schema.py`, `test_opportunities.py` (new
      file), `test_central.py`, `test_viz.py`, and `test_cli.py`
      (dangling-id rejection, valid-opportunity + central-mirror,
      empty-array no-op, HTML includes the tab, merged-view id
      namespacing, project-filter narrowing) — 136 tests total. Verified
      for real, not just via pytest: built the full example dataset
      through the real CLI, screenshotted all three tabs with headless
      Chrome (Graph shows attachment signs, Opportunities shows all 3
      cards sorted by weight with working "built from" links back to the
      Graph tab, Stats shows the full kind/origin breakdown), and
